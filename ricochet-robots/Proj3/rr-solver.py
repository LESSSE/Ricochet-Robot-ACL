...
def rr_to_mzn(file_rr,file_mzn,file_aux,file_err,t):
	...
	program = "include \"globals.mzn\";\n\n"

	program += "int: t_max = "+str(t_max)+";\n"
	program += "int: v_max = "+str(v_max)+";\n"
	program += "int: k_max = "+str(k_max)+";\n"
	program += "int: d_max = "+str(d_max)+";\n\n"

	program += "set of int: ROBOTS = 0..k_max-1;\n"
	program += "set of int: TIMESTEPS = 0..t_max-1;\n"
		
	program += "array[ROBOTS,0..t_max] of var 0..v_max: p; % _position of robot i at time j\n"
	program += "array[TIMESTEPS] of var 0..k_max: m; % _robot that moved at i timestep\n"
	program += "array[TIMESTEPS] of var 0..d_max: d; % _direction of i th movement\n"
	program += "array[0..v_max-1,0..v_max-1,TIMESTEPS] of var bool: poss; % _possible mov from i to j at time k\n"
	program += "array[0..v_max-1,0..t_max] of var bool: clear; % _clear\n\n"
	...
	for i in range(0,k_max):
		...
		y = int(l.split()[2])
		v = xy_to_v(x,y)
		program += "constraint p["+ str(color_to_k(color)) +",0] = "+str(v)+";\n"
	...
	y = int(l.split()[2])
	v = xy_to_v(x,y)
	program += "constraint p["+ str(color_to_k(color)) + ",t_max] = "+str(v)+";\n"
	...
	#file_err.write("\trr_to_mzn - 1. Positions: each robot have to have one position per time step\n")
	#positions
	#each robot have to have one position per time step 
	
	#not necessary in this implementation
	
	#file_err.write("\trr_to_mzn - 1. Positions: two robots can not have the same position at the same time\n")
	#each robot can only be at most at one position at each time step 
	
	program += "constraint forall(t in 0..t_max)(all_different([p[k,t]|k in 0..k_max-1]));\n"
		
	#file_err.write("\trr_to_mzn - 2. Clear: clear means that there is no robot in the vertex\n")
	#a vertex is clear if the position of every robot is a different vertex  

	program += "constraint forall(t in 0..t_max,v in 0..v_max-1)(clear[v,t] -> (forall(k in 0..k_max-1)(p[k,t] != v)));\n"
	program += "constraint forall(t in 0..t_max,v in 0..v_max-1)((forall(k in 0..k_max-1)(p[k,t] != v)) -> (clear[v,t]));\n"

	#file_err.write("\trr_to_mzn - 3. Possible: possible moves must not have robots in the middle and must have one at the end\n")
	...
	for e in edges:
		...
		program += "constraint forall(t in TIMESTEPS)(poss["+str(v1)+","+str(v2)+",t] -> ("
		f=0
		for v in between_xy(x1,y1,x2,y2):
			if(f==1):
				program += " /\\ "
			ve = xy_to_v(v[0],v[1])
			program += "(clear["+str(ve)+",t])"
			f=1
	...
	#file_err.write("\trr_to_mzn - 4. Moves: for each time step each robot or stay or move\n")
	#for each time step each robot or stay or move
	for v in range(0,v_max):
		poss_mv = []
		aux_s = []
		x2, y2 = v_to_x(v),v_to_y(v)
		program += "constraint forall(k in ROBOTS, t in TIMESTEPS)"
		program += "(p[k,t+1]=="+str(v)+" -> ((p[k,t]=="+str(v)+") \\/ ("
		f=0
		for e in edges:
			if e[1] == v:
				if(f==1):
				...
				program += "(d[t]=="+str(d)+" /\\ m[t]==k /\\ poss["+str(e[0])+","+str(e[1])+",t] /\\ p[k,t] == "+str(e[0])+")"
				f=1
			elif e[0] == v:
				if(f==1):
					program += " \\/ "
				...
				d = direction_to_d(direction_xy(x1,y1,x2,y2))
				program += "( d[t]=="+str(d)+" /\\ m[t]==k /\\ poss["+str(e[1])+","+str(e[0])+",t] /\\ p[k,t] == "+str(e[1])+")"
				f=1
	...
	program += "output[show(t_max), \"\\n\",\n"
	for t in range(0,t_max):
		program += "show(m["+str(t)+"]),\" \",show(d["+str(t)+"]),\"\\n\",\n"
	program += "];"
	file_mzn.write(program)

def mzn_to_sol(file_sol,file_mzn,file_aux,file_err,t):
	global n_aux
	global t_max
	...
	f=0
	for l in file_mzn.readlines():
		if ("UNSATISFIABLE" in l):
			file_err.write("\tmzn_to_sol - 1. UNSATISFIABLE | t = "+str(t_max)+"\n")
			return -1
		else:
			l = l.strip()
			l = l.split()
			if(len(l)>=2):
				moves += [str(k_to_color(int(l[0])))+" "+str(d_to_direction(int(l[1])))+"\n"]

	file_err.write("\tmzn_to_sol - 1. SATISFIABLE | t = "+str(t_max)+"\n")
...
def rr_solver(t):
		...
		f_mzn = open("problem-"+ str(t_max) +".mzn", 'w')
		f_aux = open("vars-"+ str(t_max) +".aux", 'w')
	
		#f_err.write("RR_TO_mzn | "+str(t_max)+" \n")
		rr_to_mzn(f_rr,f_mzn,f_aux,f_err,t_max)
		...
		f_err.write("RR_TO_mzn | Time: "+str(end - start)+" \n")
		...
		f_err.write("MZN_SOLVER | Time: "+str(end - start)+" \n")

		#____
		start = time.time()

		f_mzn = open("solution-"+ str(t_max) +".mzn", 'r')
		f_aux = open("vars-"+ str(t_max) +".aux", 'r')
		f_sol = open("solution-"+str(t_max)+".sol", 'w')

		#f_err.write("mzn_TO_SOL\n")
		sat = mzn_to_sol(f_sol,f_mzn,f_aux,f_err,t_max)

		end = time.time()

		f_err.write("mzn_TO_SOL | Time: "+str(end - start)+" \n")
...
if __name__ == "__main__":
	...
	os.system("rm *.mzn *.aux problem.rr solution*.sol")
