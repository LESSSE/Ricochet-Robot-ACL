...
def readSolution(in_file, out_file, max_time=4):
	robotTime_to_pos = [[] for i in range(max_time*4)]
	
	line = in_file.readline()
	while("moved" not in line):
		line=in_file.readline()
		continue
	...
	while("position" not in line):
		...
		if( len(tokens)<8 or "false" in tokens[8]):
			continue
		moved_robots.append(tokens[4][0])
	line = in_file.readline()
	while("))))" not in line): #FIXME
		tokens = filter(None, line.split(' '))
		if( "false" in tokens[14]):
			line=in_file.readline()
			continue
		line = in_file.readline()
		
		pos_line = readInt(tokens[4])
		pos_col = readInt(tokens[7])
		pos_rob = readInt(tokens[10])
		pos_time = readInt(tokens[13])
		
		robotTime_to_pos[pos_rob+4*pos_time] = [pos_line, pos_col]
	
	#print(str(max_time-1)+"\n")
	out_file.write(str(max_time-1)+"\n")
	for time in range(1, max_time):
		robot = int(moved_robots[time-1])
		pos_dest = robotTime_to_pos[robot + time*4]
		pos_init = robotTime_to_pos[robot + (time-1)*4]
		#print(robotToString(str(robot)) +" " +getDirection(pos_init, pos_dest)+"\n")
		out_file.write(robotToString(str(robot)) +" " +getDirection(pos_init, pos_dest)+"\n")
...
def smt_to_sol(file_sol,file_smt,file_aux,file_err,t):
	res=file_smt.readline()
	sys.stderr.write("First line of solution file: "+res+"\n")
	if("unsat" not in res and "sat" in res): #FIXME should probably do this properly. And solve the cae of t==0
		sys.stderr.write("SAT for " + str(t) + " timesteps!\n")
		readSolution(file_smt, file_sol, t)
		return 1
	return 0
	#return 1
...
def rr_solver(t):
		...
		Encoder.writeEverything(f_rr, f_smt, t_max)
...
if __name__ == "__main__":
	...
	#sys.stderr.write("\nt_unsat:")
	#sys.stderr.write(str(t_unsat))
	#sys.stderr.write("\nt_sat:")
	#sys.stderr.write(str(t_sat))
	#sys.stderr.write("\nt:")
	#sys.stderr.write(str(t))
	#sys.stderr.write("\n")
	
	while(t != t_unsat):
		sys.stderr.write("Time before solver call: "+str(t)+"\n")
		sat = rr_solver(t)
		sys.stderr.write("Time after solver call: "+str(t)+"\n")
